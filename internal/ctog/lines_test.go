...
func TestParseCtoGLines(t *testing.T) {
	...
	t.Run("CII_example1.xml", func(t *testing.T) {
		...
		assert.Equal(t, num.NewAmount(995, 2), lines[0].Item.Price)
		...
		assert.Equal(t, num.NewAmount(985, 2), lines[1].Item.Price)
		...
		assert.Equal(t, num.NewAmount(829, 2), lines[2].Item.Price)
